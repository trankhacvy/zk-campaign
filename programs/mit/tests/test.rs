...
use mit::CounterCompressedAccount;
...
async fn test() {
    let (mut rpc, env) =
        setup_test_programs_with_accounts_v2(Some(vec![(String::from("mit"), mit::ID)])).await;
    ...
    R: RpcConnection + MerkleTreeExt,
{
    ...
    println!("address_merkle_context {}", merkle_context);

    let instruction_data = mit::instruction::Create {
        inputs: Vec::new(),
    ...
    let cpi_signer = find_cpi_signer(&mit::ID);

    let accounts = mit::accounts::Create {
        signer: payer.pubkey(),
    ...
    R: RpcConnection + MerkleTreeExt,
{
    ...
    let instruction_data = mit::instruction::Increment {
        inputs,
        proof: rpc_result.proof,
        merkle_context,
    ...
    let accounts = mit::accounts::Increment {
        signer: payer.pubkey(),
        light_system_program: PROGRAM_ID_LIGHT_SYSTEM,
        account_compression_program: PROGRAM_ID_ACCOUNT_COMPRESSION,
        account_compression_authority,
    ...
    R: RpcConnection + MerkleTreeExt,
{
    ...
    let instruction_data = mit::instruction::Delete {
        inputs,
        proof: rpc_result.proof,
        merkle_context,
        merkle_tree_root_index: rpc_result.root_indices[0],
    ...
    let accounts = mit::accounts::Delete {
        signer: payer.pubkey(),
        light_system_program: PROGRAM_ID_LIGHT_SYSTEM,
        account_compression_program: PROGRAM_ID_ACCOUNT_COMPRESSION,
        account_compression_authority,
